  ...
  template: `
    <div class="inventory-studio-container p-6 animate-fade-in" [class.embedded]="inlineMode">
        ...
        <div class="space-y-3">
          ...
          <div>
            ...
            <p class="mt-2 max-w-2xl text-sm font-medium text-slate-500">
              Manage item identity, pricing, tax settings, stock rules, and variants in the same premium form layout used by Client Sales Studio.
            </p>
          </div>
        </div>
      ...
      @if (loadingItem()) {
        ...
        <form [formGroup]="form" (ngSubmit)="submit()" class="studio-layout grid grid-cols-1 gap-8 xl:grid-cols-[1fr_360px]">
  ...
  styles: [`
    ...
    .inventory-studio-container {
      ...
      background:
        ...
        linear-gradient(180deg, #f8fffc 0%, #f8fafc 42%, #ffffff 100%);
    }
    ...
    .glass-card-primary {
      ...
      background: linear-gradient(135deg, #10b981, #065f46 70%, #064e3b 100%);
      border-color: rgba(var(--studio-primary-rgb), 0.28);
      box-shadow: 0 24px 55px -30px rgba(var(--studio-primary-rgb), 0.7);
    }

    .section-icon {
      @apply flex h-10 w-10 items-center justify-center rounded-2xl border text-slate-500;
    ...
    .studio-input {
      @apply w-full rounded-2xl border px-4 py-3 text-sm font-medium text-slate-900 transition-all placeholder:text-slate-300 focus:outline-none;
      background: #f8fafc;
      border-color: #e2e8f0;
    }
    ...
    .btn-studio-primary {
      @apply inline-flex items-center gap-2 rounded-2xl px-6 py-3 font-bold text-white transition-all disabled:opacity-60;
      background-color: var(--studio-primary);
      box-shadow: 0 12px 20px -10px var(--studio-primary-glow);
    }
    ...
    .btn-studio-secondary {
      @apply inline-flex items-center gap-2 rounded-2xl px-5 py-3 font-bold text-slate-600 transition-all;
      background: #f1f5f9;
    }
    ...
    .studio-checkbox {
      @apply mt-1 h-4 w-4 shrink-0 accent-emerald-500;
    }
    ...
    .summary-chip span {
      @apply block text-[10px] font-black uppercase tracking-[0.18em] text-white/55;
    }

    .summary-chip strong {
      @apply mt-1 block text-sm font-bold text-white;
    }
    ...
    .check-row.complete .check-dot {
      ...
      box-shadow: 0 0 0 4px rgba(var(--studio-primary-rgb), 0.14);
    }
