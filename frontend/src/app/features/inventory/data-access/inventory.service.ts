...
export class InventoryService {
  ...
  adjustStock(dto: {
    ...
    variantId?: string; reason?: string; notes?: string; clientId?: string;
  }): Observable<any> {
  ...
  getPurchaseOrders(filters: {
    ...
    return this.http.get(`${this.base}/purchase-orders`, { params }).pipe(
      map((res: any) => ({ ...res, total: res.meta?.total ?? res.total ?? 0 })),
    );
  }
  ...
  autoCreatePOsForLowStock(clientId: string, warehouseId: string): Observable<any> {
    return this.http.post(`${this.base}/purchase-orders/auto-create/${clientId}`, { warehouseId });
  }
